...
class ChatSession:
    ...
    # Метод для вставки внутрь класса ChatSession
    async def send_file(self, file_path: str | Path) -> int | None:
        """
        Отправляет файл через Ctrl+V. 
        Файл улетает сразу после вставки, подпись не предусмотрена.
        """
        path = Path(file_path)
        if not path.exists():
        ...
        # Определяем MIME-тип или используем универсальный
        ...
        try:
            ...
            # Записываем именно объект File в буфер обмена, чтобы сохранилось имя
            await self._page.evaluate(
                """
                async ([b64, mimeType, fileName]) => {
            ...
            # Вставляем — в Telegram Web это триггерит мгновенную отправку
            await self._page.keyboard.press("Control+V")
            
            # Ждем появления нового индекса в чате (таймаут чуть больше для тяжелых файлов)
            new_index = await _wait_for_new_index(self._page, before, timeout=15.0)
            print(f"[{self._chat_name}] Файл '{path.name}' отправлен [idx={new_index}]")
            return new_index
            
        except Exception as e:
            print(f"[{self._chat_name}] Ошибка при отправке файла: {e}")
            return None

    async def send_image(self, image_path: str | Path, text: str | None = None) -> int | None:
        """
        Отправляет изображение (опционально с текстом) через Ctrl+V.
        Возвращает data-index нового сообщения (или None при ошибке).
        """
        ...
        try:
            ...
            await self._page.keyboard.press("Enter")
            new_index = await _wait_for_new_index(self._page, before, timeout=5.0)
            print(f"[{self._chat_name}] Отправлено: {path.name}" + (f" + {text!r}" if text else "") + f" [idx={new_index}]")
            return new_index
        except Exception as e:
